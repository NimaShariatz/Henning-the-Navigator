...
interface MinimapProps {
    ...
    targets_set: {id: number, x: number, y: number, type: number, targetName: string, isBlue: boolean}[]; // Add this
...
function Minimap({ 
    ...
    const calculate_target_for_minimap = (target: {id: number, x: number, y: number, type: number, isBlue: boolean}) => {
        const main_map_image_element = document.querySelector('canvas.map_background');
        ...
        const relative_X = adjusted_x / main_map_width;
        const relative_Y = adjusted_y / main_map_height;
        ...
        return {
            x: minimap_X,
            y: minimap_Y,
            minimap_target_size: targetSize
        };
    };

    const get_minimap_target_icon = (type: number, isBlue: boolean) => {
        ...
        switch(type) {
                ...
                return (
                    <svg xmlns="http://www.w3.org/2000/svg" width="85%" height="85%" viewBox="0 0 24 24">
                        <g fill="none">
                            ...
                            <path fill={fillColor} className="target_button_fill" d="M5.269 4.026c.557-.628 1.463-.628 2.044-.148l.105.095l4.848 4.847l.087-.087a1 1 0 0 1 
            ...
            case 4:
                return(
                    <svg xmlns="http://www.w3.org/2000/svg" width="75%" height="75%"  viewBox="0 0 30 30">
        ...
        <>
            ...
            <div className="minimap_container" style={{display: showMinimap ? 'flex' : 'none'}}>
                <div className="navigation_container">
                    ...
                    {/* Render targets */}
                    {targets_set.map(target => {
                        const pos = calculate_target_for_minimap(target);
                        return (
                            <div 
                                key={`target-${target.id}`} 
                                className="minimap_target"
                                style={{
